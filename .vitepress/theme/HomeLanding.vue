  ...
  <section class="figma-landing" aria-labelledby="site-hero-title">
    <section class="figma-hero">
      <button
        ...
        @click="toggleTheme"
      >
        <span class="figma-theme-toggle__icon" aria-hidden="true"></span>
      </button>
      ...
      <div class="figma-wrap figma-split">
        <div>
          ...
          <div class="figma-tags" aria-label="核心能力">
            <span v-for="tag in coreTags" :key="tag">{{ tag }}</span>
          </div>
        </div>
        ...
        <div class="figma-product" aria-label="智行租赁运营看板示意">
          ...
          <div class="figma-product__body">
            <div class="figma-product__metrics">
              <article v-for="item in dashboardMetrics" :key="item.title" class="figma-card">
            ...
            <div class="figma-product__cards">
              <article v-for="item in dashboardCards" :key="item.title" class="figma-card">
                <span class="figma-label">{{ item.label }}</span>
                <strong>{{ item.title }}</strong>
                <p>{{ item.text }}</p>
              </article>
    ...
    <section class="figma-section figma-section--cream">
      <div class="figma-wrap">
        ...
        <div class="figma-grid figma-grid--4">
          <article v-for="item in outcomes" :key="item.title" class="figma-card figma-card--numbered">
        ...
        <div class="figma-grid figma-grid--3 figma-grid--mt" aria-label="经营闭环补充内容">
          <article v-for="item in outcomeDetails" :key="item.title" class="figma-card">
            <span class="figma-label">{{ item.label }}</span>
            <strong>{{ item.title }}</strong>
            <p>{{ item.text }}</p>
          </article>
    ...
    <section class="figma-section figma-section--soft" aria-labelledby="workflow-title">
      <div class="figma-wrap">
        ...
        <ol class="figma-grid figma-grid--6 figma-workflow">
          <li v-for="(step, index) in workflow" :key="step.title" class="figma-card figma-card--numbered">
        ...
        <div class="figma-grid figma-grid--3 figma-grid--mt" aria-label="流程补充信息">
          <article v-for="item in workflowMeta" :key="item.title" class="figma-card">
            <span class="figma-label">{{ item.label }}</span>
            <strong>{{ item.title }}</strong>
            <p>{{ item.text }}</p>
          </article>
    ...
    <section class="figma-section figma-section--lilac" aria-labelledby="capability-title">
      <div class="figma-wrap">
        ...
        <div class="figma-grid figma-grid--4">
          <article v-for="item in capabilities" :key="item.title" class="figma-card">
            <h4>{{ item.title }}</h4>
            <p>{{ item.text }}</p>
    ...
    <section class="figma-section figma-section--navy" aria-labelledby="roles-title">
      <div class="figma-wrap">
        ...
        <div class="figma-grid figma-grid--4" aria-label="角色工作台摘要">
          <article v-for="item in roleBoard" :key="item.title" class="figma-card">
            <span class="figma-label">{{ item.label }}</span>
            <strong>{{ item.title }}</strong>
          </article>
        </div>
        ...
        <div class="figma-grid figma-grid--3 figma-grid--mt">
          <article v-for="role in roles" :key="role.name" class="figma-card">
    ...
    <section class="figma-section figma-section--pink" aria-labelledby="plans-title">
      <div class="figma-wrap">
        ...
        <div class="figma-grid figma-grid--3">
          <article v-for="plan in plans" :key="plan.name" class="figma-card">
        ...
        <div class="figma-next">
          <div>
            <p class="figma-copy">理性评估先看功能，想快速感受产品就进演示，也可以扫码咨询演示账号。</p>
          </div>
          <div class="figma-grid figma-grid--3">
            <article class="figma-card figma-card--contact">
              <div>
                <span class="figma-label">微信咨询</span>
                <p>扫码领演示账号，咨询价格。</p>
              </div>
              <img :src="wechatQrSrc" alt="智行租赁微信联系方式二维码" />
            </article>
            <article class="figma-card">
              <span class="figma-label">理性评估</span>
              <p>先确认车辆、订单、财务、风控和租客端能力是否覆盖当前业务。</p>
              <a class="figma-button figma-button--primary" href="/overview/what-features">先看功能</a>
            </article>
            <article class="figma-card">
              <span class="figma-label">体验产品</span>
              <p>直接进入演示站点，按签约、出车、退车结算体验管理端流程。</p>
              <a class="figma-button figma-button--magenta" :href="demoUrl" target="_blank" rel="noreferrer">进入演示</a>
            </article>
          </div>
        </div>
...
const coreTags = ['车辆资产', '租赁订单', '财务收付', '风控留痕', '合同单据', '企业微信通知', '租客端小程序', '独立部署']
